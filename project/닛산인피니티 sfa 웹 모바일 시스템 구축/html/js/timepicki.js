...
(function($) {
	...
	$.fn.timepicki = function(options) {
		...
		return this.each(function() {
			...
			$('.reset_time').on("click touchstart", function(event) {
			...
			function set_date(start_time) {
				...
				ele_next.find(".ti_tx input").val("0" + 7);
				if(settings.finishTime=="Y"){
					ele_next.find(".mi_tx input").val("0" + 9);
				}else{
			...
			function change_mins(cur_ele, direction) {
				...
				var finishTime = settings.finishTime;
				...
				if ((cur_ele && cur_ele.hasClass('action-next')) || direction === 'next') {
					if (cur_mins + step_size > ele_en) {
						ele_next.find("." + cur_cli + " .mi_tx input").val("09");
